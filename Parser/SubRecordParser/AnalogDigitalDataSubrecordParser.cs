...
{
	internal class AnalogDigitalDataSubrecordParser:ISubrecordParser
	{
		...
		{
			var ret = new EgtsAnalogDigitalData();

			byte byt = 0;
			if (!br.ReadNextByteBytes(ref byt, dataArray, ref remainsize, ref offset))
				return null;
			parseBitsToBools(ret.DIOE, 0, byt);
			if (!br.ReadNextByteBytes(ref byt, dataArray, ref remainsize, ref offset))
				return null;
			parseBitsToBools(ret.DOUT, 0, byt);
			if (!br.ReadNextByteBytes(ref byt, dataArray, ref remainsize, ref offset))
				return null;
			parseBitsToBools(ret.ASFE, 0, byt);
			...
			for (int i = 0; i < 8; i++)
			{
				...
				if (!br.ReadNextByteBytes(ref byt, dataArray, ref remainsize, ref offset))
					return null;
				parseBitsToBools(ret.ADIO, i*8, byt);
			}
