...
namespace Transport.EGTS.Helpers
{
	...
	internal class CrcHelper
	{
		...
		*/
		...
		/// <param name="headerBitsByte">The header bits byte.</param>
		/// <param name="encoded_size">The encoded_size.</param>
		/// <returns></returns>
		internal byte CalculateHeaderCrc(EgtsPacketHeader phead, byte[] pecnoded_part, byte headerBitsByte, int encoded_size)
		{
			byte crc = 0xFF, puc;
			crc = (byte) crc8_table[crc ^ phead.PRV];
			crc = (byte) crc8_table[crc ^ phead.SKID];
			crc = (byte) crc8_table[crc ^ headerBitsByte];
			crc = (byte) crc8_table[crc ^ phead.HL];
			crc = (byte) crc8_table[crc ^ phead.HE];

			for (var i = 0; i < encoded_size; i++)
			{
				puc = pecnoded_part[i];
				crc = (byte) crc8_table[crc ^ puc];
			}

			return crc;
		}

		internal byte checkCrc8(byte[] pecnoded_part)
		{
			byte crc = 0xFF;
			for (int i = 0; i < pecnoded_part.Length; i++)
			{
				var b = pecnoded_part[i];
				crc = (byte) crc8_table[crc ^ b];
			}
			return crc;
		}
		...
		/// <summary>
		/// Calculates the data CRC.
		/// egts_data_crc()
		/// </summary>
		/// <param name="pbuf">The pbuf.</param>
		/// <param name="sz">The size.</param>
		/// <returns></returns>
		internal ushort CalculateDataCrc(byte[] pbuf, short sz)
		{
			var crc = (ushort) 0xFFFF;
			var i = 0;
			while (0U != sz--)
			{
				crc = (ushort) ((crc << 8) ^ crc16_table[(crc >> 8) ^ pbuf[i]]);	// not quite sure...
				i++;
			}
			return crc;
		}
